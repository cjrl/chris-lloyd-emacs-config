...
#+begin_src emacs-lisp
(defun mathlib4-run-ci-checks ()
  "Run lake build, style checks, and environment linters for the current Lean file, 
displaying results in a bottom-docked compilation buffer."
  (interactive)
  (unless (buffer-file-name)
    (error "This buffer is not visiting a file!"))
  
  (let* ((filepath (buffer-file-name))
         (raw-root (locate-dominating-file filepath "lakefile.lean"))
         
         (_ (unless raw-root 
              (error "Could not find lakefile.lean in any parent directory!")))
              
         (project-root (expand-file-name raw-root))
         (rel-path (file-relative-name filepath project-root))
         
         (module (replace-regexp-in-string 
                  "/" "." 
                  (file-name-sans-extension rel-path)))
         
         (cmd (format "cd %s && lake build && lake exe lint-style && lake exe runLinter %s"
...
#+begin_src emacs-lisp
(defun lean-refresh-mathlib ()
  "Kill running Lean processes and fetch the latest mathlib cache for the current project."
  (interactive)
  (let* ((root (or (locate-dominating-file default-directory "lakefile.lean")
                   (locate-dominating-file default-directory "lean-toolchain")
                   default-directory))
         (default-directory root))
    (message "Stopping Lean processes...")
    (shell-command "killall lean")
    
    (message "Fetching mathlib cache in project root: %s" root)
    (async-shell-command "lake exe cache get" "*lean-cache-get*")))
#+end_src
...
#+begin_src emacs-lisp
  (my-leader-def 'lean4-mode-map
    ...
    "l r" 'lean-refresh-mathlib
    "c" 'mathlib4-run-ci-checks
    "?" 'lsp-ui-doc-mode)
#+end_src
