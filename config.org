...
#+begin_src emacs-lisp :lexical t 
  ...
  (setq my-lean-symbols
    ...
    (add-hook 'lean4-mode-hook
		...
		(setq-local company-minimum-prefix-length 1)) 90)

#+end_src

#+RESULTS:
| (lambda nil (set (make-local-variable 'company-backends) '((company-my-lean-symbols company-dabbrev company-dabbrev-code company-files company-capf company-keywords company-semantic company-clang))) (set (make-local-variable 'company-minimum-prefix-length) 1)) | my-lean-key-chord-fix | lsp | (closure (t) nil (setq-local company-backends '((company-my-lean-symbols company-dabbrev company-dabbrev-code company-files company-capf company-keywords company-semantic company-clang))) (setq-local company-minimum-prefix-length 1)) |
